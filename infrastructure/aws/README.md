...
Prerequisites: AWS CLI credentials, Docker, an ECR repository, and an HTTPS OIDC issuer.
...
aws cloudformation deploy \
  ...
  --parameter-overrides \
    ...
    OAuth2IssuerUri="https://YOUR-IDP.example.com/" \
    AlarmEmail="YOUR-EMAIL@example.com"
```

Confirm the SNS email subscription, wait for the App Runner service to become `RUNNING`, then use
the stack outputs for its URL, dashboard, alarm topic, and private database endpoint.
...
- The generated password is visible only to the App Runner instance role and authorized operators.
